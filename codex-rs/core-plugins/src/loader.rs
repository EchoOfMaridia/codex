...
pub(crate) async fn load_plugin_capability_facts(
    ...
    let mcp_discovery = match &manifest_paths.mcp_servers {
        Some(PluginManifestMcpServers::Object(object_servers)) => {
            load_mcp_servers_from_manifest_object(plugin_root.as_path(), object_servers)
        }
        Some(PluginManifestMcpServers::Path(_)) | None => {
            ...
            for path in plugin_mcp_config_paths(plugin_root.as_path(), manifest_paths) {
                let file_discovery = load_mcp_servers_from_file(plugin_root.as_path(), &path).await;
                discovery.had_errors |= file_discovery.had_errors;
                discovery.mcp_servers.extend(file_discovery.mcp_servers);
            }
            discovery
        }
    };
    had_errors |= mcp_discovery.had_errors;
    let mut mcp_server_names = mcp_discovery.mcp_servers.into_keys().collect::<Vec<_>>();
