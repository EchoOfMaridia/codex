...
use super::agent_identity::MAX_AGENT_IDENTITY_BOOTSTRAP_ATTEMPTS;
use super::agent_identity::RetryableAgentIdentityRegistrationError;
use super::agent_identity::is_retryable_io_registration_error;
use super::agent_identity::retry_registration;
...
impl CodexAuth {
    async fn from_auth_dot_json(
        ...
        if auth_mode == ApiAuthMode::AgentIdentity {
            ...
            let base_url = chatgpt_base_url
                ...
                .trim_end_matches('/')
                .to_string();
            let agent_identity_authapi_base_url = resolve_agent_identity_authapi_base_url(
                Some(&base_url),
    ...
    pub async fn from_agent_identity_jwt(
        ...
        Ok(Self::AgentIdentity(
            AgentIdentityAuth::from_jwt(jwt, &base_url, &resolved_authapi_base_url).await?,
        ))
    }
    ...
    fn stored_chatgpt_agent_identity_record(
        &self,
        account_id: &str,
    ...
    fn persist_chatgpt_agent_identity_auth(
        &self,
        agent_identity_auth: AgentIdentityAuth,
    ) -> std::io::Result<AgentIdentityAuth> {
        if let Self::Chatgpt(chatgpt_auth) = self {
            chatgpt_auth.persist_agent_identity_record(agent_identity_auth.record().clone())?;
        }
        Ok(agent_identity_auth)
    }
    ...
    async fn ensure_chatgpt_agent_identity(
        ...
        if let Some(record) = self.stored_chatgpt_agent_identity_record(&binding.account_id)
        ...
        {
            let auth = map_retry_exhaustion_to_bootstrap_unavailable(
                "agent task registration",
                AgentIdentityAuth::from_record(record, &agent_identity_authapi_base_url).await,
            )?;
            return self.persist_chatgpt_agent_identity_auth(auth);
        }

        let key_material = generate_agent_key_material().map_err(std::io::Error::other)?;
        let client = build_reqwest_client();
        let runtime_id = map_retry_exhaustion_to_bootstrap_unavailable(
        ...
        self.persist_chatgpt_agent_identity_auth(auth)
    }
...
fn agent_identity_bootstrap_unavailable_error(
    operation: &'static str,
    err: std::io::Error,
) -> std::io::Error {
...
fn map_retry_exhaustion_to_bootstrap_unavailable<T>(
    operation: &'static str,
    result: std::io::Result<T>,
) -> std::io::Result<T> {
    result.map_err(|err| {
        if is_retryable_io_registration_error(&err) {
            agent_identity_bootstrap_unavailable_error(operation, err)
        } else {
            err
        }
    })
...
pub async fn login_with_access_token(
    ...
    let _ = agent_identity_authapi_base_url;
...
pub trait AuthManagerConfig {
    ...
    /// Returns an explicit AuthAPI base URL for Agent Identity registration, when configured.
...
impl AuthManager {
    ...
    pub fn set_forced_chatgpt_workspace_id(&self, workspace_id: Option<Vec<String>>) {
        let changed = if let Ok(mut guard) = self.forced_chatgpt_workspace_id.write()
            && *guard != workspace_id
        {
            *guard = workspace_id;
            true
        ...
        let should_clear_cached_auth = self
            .inner
            .read()
            .ok()
            .and_then(|guard| guard.auth.clone())
            .is_some_and(|auth| match auth {
                CodexAuth::ApiKey(_) | CodexAuth::BedrockApiKey(_) => false,
                _ => {
                    let expected_workspace_ids = self.forced_chatgpt_workspace_id();
                    let Some(expected_workspace_ids) = expected_workspace_ids.as_deref() else {
                        return false;
                    };
