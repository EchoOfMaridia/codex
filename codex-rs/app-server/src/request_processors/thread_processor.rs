...
impl ThreadRequestProcessor {
    ...
    async fn thread_start_task(
        ...
        let multi_agent_mode = thread.effective_multi_agent_mode().await;
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let multi_agent_mode = codex_thread.effective_multi_agent_mode().await;
                let sandbox = thread_response_sandbox_policy(
                    &config_snapshot.permission_profile,
                    config_snapshot.cwd().as_path(),
                );
    ...
    async fn thread_fork_inner(
        ...
        let config_snapshot = forked_thread.config_snapshot().await;
        let multi_agent_mode = forked_thread.effective_multi_agent_mode().await;
        let sandbox = thread_response_sandbox_policy(
            &config_snapshot.permission_profile,
            config_snapshot.cwd().as_path(),
        );
